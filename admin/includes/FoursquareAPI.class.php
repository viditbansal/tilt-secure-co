...
class FoursquareApi {
	...
	private function Request($url, $params=false, $type=HTTP_GET){
		...
		if($type == HTTP_GET) $url = urldecode($this->MakeUrl($url,$params));
	...
	private function RequestData($url, $params=false, $type=HTTP_GET){
	 ...
	 */
	private function GETdata($url,$params=false){
		$url = urldecode($this->MakeUrl($url,$params));
		return $this->RequestData($url,$params,HTTP_GET);
	}
	/**
	 * GET
	 * Abstraction of the GET request
