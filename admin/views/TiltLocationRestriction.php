...
if(isset($_POST['usId']) &&	$_POST['usId'] !=''){
	...
	header("location:TiltLocationRestriction?cs=1");
		die();
}
$fields			=	' rc.*,c.Country,s.State ';
$condition		=	' rc.Status = 1 ';
$countriesListResult		=	$coinsManageObj->getTiltRestCountries($fields,$condition);
...
<body >
	...
	<div class="box-header">
		<h2><i class="fa fa-list"></i>Location restriction to buy/play with TiLT$</h2>
		<span class="fright">

			<a href="TiltManageLocationRestriction" class="restrictCountries" title="Restrict Countries"><i class="fa fa-plus-circle"></i> Restrict Countries</a>
		</span>
	</div>
							...
							?>				<div  id="<?php echo $countryKey; ?>" class="res_countries_delete rest_country<?php echo $countryKey; ?>" >
												...
												<input type="hidden"  name="usId" value="<?php echo $countryKey; ?>" />
													<a onclick="javascript:if(confirm('Are you sure want to delete some states belongs to this country?')) $('#remove_us').submit();" title="Delete" class="remove"><i class="fa fa-times-circle"></i></a>
														<?php }else { ?>
															<a onclick="remove_country(<?php echo $countryKey ?>,'tilt_location_restrict');" class="remove" title="Delete"><i class="fa fa-times-circle"></i></a>
														<?php } ?>
												</form>	
											</div>
							<?php		} 
					...
					<tr><td height="20"></td></tr>
						<tr id="state_block">
							...
							?>				<div  id="<?php echo $stateKey; ?>" class="res_countries_delete rest_state<?php echo $stateKey; ?>" >
												...
												<a onclick="remove_state(<?php echo $usId; ?>,<?php echo $stateKey; ?>,'tilt_location_restrict');" class="remove" title="Delete"><i class="fa fa-times-circle"></i></a>
											</div>
							<?php		} 
									}
									else echo ' - ';
