...
//get developer page content
$fields		  =	" * ";
$where		  =	" id = 1 ";
$developer_details = $adminControllerObj->getWebsiteDeveloper($fields, $where);
if(isset($developer_details) && is_array($developer_details) && count($developer_details)>0){
	foreach($developer_details as $key=>$value){
...
if(isset($_POST['website_developer_submit']) && $_POST['website_developer_submit'] != '' ){
	...
	if(isset($_POST['promo_video_upload']) && $_POST['promo_video_upload'] !=''){
		...
		$promoVideoName  = 'developervideo_'.time().'.'.$ext;
	...
	if(isset($_POST['video_image_upload']) && $_POST['video_image_upload'] !=''){
		...
		$promoimageName = 'developervideoimage_'.time().'.'.$ext;
	...
	if(isset($_POST['bg_image_upload']) && $_POST['bg_image_upload'] !=''){
		...
		$bgimageName 	= 'developerbgimage_'.time().'.'.$ext;
	...
	if(isset($_POST['info_image_upload']) && $_POST['info_image_upload'] !=''){
		...
		$infoimageName 	= 'infoimage_'.time().'.'.$ext;
	...
	$adminControllerObj->updateWebsiteDeveloper($updateString,$condition);
...
<body>
<?php top_header(); ?>	
	<div class="box-header"><h2><i class="fa fa-pencil-square-o" ></i>Developer</h2></div>
	<div class="clear"></div>
	<form name="website_developer_form" id="website_developer_form" action="" method="post" enctype="multipart/form-data">
		...
		<tr><td align="center">
			<table cellpadding="0" cellspacing="0" align="center" border="0" width="80%">	
				...
				<?php displayNotification('Developer Content '); ?>
				</td></tr>
				<tr>
				...
				<tr><td height="20" colspan="2"></td></tr>
				<tr>
					<td align="left" valign="top"><label>Integration Content <span class="required_field">*</span></label></td>
					<td align="center" class="" valign="top">:</td>
					<td height="60" valign="top" align="left">
				...
				<td colspan="2"></td>
				<td align="left">
					<input type="submit" class="submit_button" name="website_developer_submit" id="website_developer_submit" value="Submit" title="Submit" alt="Submit" />
