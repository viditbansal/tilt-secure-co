...
if(isset($_POST['country'])	&&	!empty($_POST['country']))
{
	...
	if($countryId == $usId){
		if(isset($_POST['state'])	&&	is_array($_POST['state'])	&&	count($_POST['state'])>0){
			...
			if($stateIds ==''){
				...
				$usEntryResult		=	$coinsManageObj->checkCountryEntry($fields,$condition);
				$newEntry		=	"";
				...
				if($newEntry !=''){ //insert new entry
					...
					$coinsManageObj->insertRestrictedCountries($stateValues);
				}
				...
				if($setActive !=''){ //Update old entry status
					...
					$usEntryResult		=	$coinsManageObj->updateCountryStatus($updateString,$condition);
				}
			}
		}
	}
	...
	if(isset($countryIds)	&&	is_array($countryIds) &&	count($countryIds) > 0){ // Edit with more than one countries
		...
		$countryResult		=	$coinsManageObj->checkCountryEntry($fields,$condition);
	...
	else if($countryId != $usId)
	{
		...
		if(isset($usEntryResult) && is_array($usEntryResult) && count($usEntryResult) > 0){ // check entry exist
			...
			$coinsManageObj->insertRestrictedCountries($newEntry);
		}
	} 
	...
	<script type="text/javascript">
		window.parent.location.href = 'LocationRestriction';
	</script>	
<?php 	
}
//ManageRestriction
...
$condition		=	'  Status = 1 ';
$selected		=	$coinsManageObj->checkCountryEntry($fields,$condition);
...
<body>
	<div class="box-header">
		<h2><i class="fa fa-plus-circle"></i>Restrict Country</h2>
	</div>
	<div class="clear">
	<form name="loc_restriction_form" id="loc_restriction_form" action="" onSubmit="return validateState();" method="post">
					...
					<tr> <td height="10"> </td></tr>
						<tr id="state_block" style="display:none">
							...
							<td align="left"  height="40"  valign="top">
								...
								<select name="state[]" id="state" class="input" multiple  style="height:100px;display:block;">
									<option value="">Select</option>
								...
								<div id="state_empty" style="clear : both;display:none;color:red">State is required</div>
							</td>
						</tr>
				</table>
				</td>
