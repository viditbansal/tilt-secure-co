...
$tot_rec ='';
$tournamentResult  	= 	$reportObj->getUserswinTournamentreport($fields,$condition);
$tot_rec			=	$reportObj->getTotalRecordCount();
$today	= date('Y-m-d H:i');
?>
<body class="popup_bg" style="">
	<section class="content-header">
		<h2 id="heading_title"><i class="fa fa-list"></i>&nbsp;Total Tournaments <?php if(isset($_GET['type'])){if($_GET['type'] == 3) echo 'Won ';if($_GET['type'] == 4) echo 'Loss ';}  
		if(isset($_GET['gameName']) && $_GET['gameName'] != '')	echo ' - '.$_GET['gameName']; ?></h2>
	</section>	
	<section class="content col-md-12 box-center develop_page">
		
		<div class="search_group">
			<div style="height: 20px" class="clear"></div>
			<form class="tournament" method="post" action="<?php echo 'UserwinlossReport'.$pagingParam;?>">
		...
		<div id="pagingResult" class="clear" style="width: 100%">
			<form action="UserwinlossReport" class="l_form" name="TournamentPlayedUsersList" id="TournamentPlayedUsersList"  method="post"> 
		...
		<div class="col-xs-12 clear">
			<?php if(isset($tournamentResult) && is_array($tournamentResult) && count($tournamentResult) > 0 ) {
				pagingControlLatest($tot_rec,'UserwinlossReport'.$pagingParam); ?>
			<?php }?>
		</div><br>
		</section>
<?php commonFooter(); ?>
